(ns authentication.authorize-flows.flex
  (:require
...
(defn ensure-flex-role 
  "Ensure user has flex role, assign if missing"
  [user-id]
  (let [user-roles (mapv :role (user-db/get-user-roles user-id))
        has-flex? (some #(= "flex" %) user-roles)]
    (when-not has-flex?
      (user-db/add-user-role (str (java.util.UUID/randomUUID)) user-id "flex"))
    ;; Return updated roles list
    (if has-flex?
      user-roles
      (conj user-roles "flex"))))

(defn handle-flex-authorization 
  "Handle flex authorization flow - automatically creates users as flex users"
  ...
  (if (nil? auth0-token)
    ...
    (let [sub         (:sub auth0-token)
      ...
      (cond 
        user-id
        (let [user-roles (ensure-flex-role user-id)]
          (merge-to-session
           (helpers/redirect-handler (or redirect-url "/flex"))
