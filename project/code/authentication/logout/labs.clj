(ns authentication.logout.labs
  (:require
   [zero.backend.state.env :as env]))

(def logout-route "/logout/labs")

(defn logout-handler 
  "Handle labs logout by clearing session and redirecting to Auth0 logout"
  [req]
  (let [domain-value @env/domain
        return-url (str domain-value "/irunrainbow")
        client-id @env/auth0-labs-client-id
